defmodule ANN.Layer do
  ...
  def process({layer, values, activation_fn}) do
    neurons =
      layer.neurons
      |> Enum.map(&process_neuron(&1, values, layer.bias, activation_fn))
  ...
  defp process_neuron(neuron, values, bias, activation_fn) do
    Task.async(fn -> Neuron.process(neuron, values, bias, activation_fn) end)
  end
end
