defmodule ANNEx.Training.Backpropagation.UpdateWeights do
  ...
  defp update_layer(%Layer{}=layer, learning_rate, activation_fn) do
    neurons =
      ...
      |> Enum.map(&Task.async fn ->
        update_neuron(&1, layer.bias, learning_rate, activation_fn)
      end)
      |> Enum.map(&Task.await/1)
  ...
  defp update_neuron(%Neuron{}=neuron, bias, learning_rate, activation_fn) do
    signals =
      neuron.signals
      |> Enum.map(&Task.async fn ->
        update_signal(&1, neuron.delta, neuron.sum, learning_rate, activation_fn)
      end)
      |> Enum.map(&Task.await/1)
